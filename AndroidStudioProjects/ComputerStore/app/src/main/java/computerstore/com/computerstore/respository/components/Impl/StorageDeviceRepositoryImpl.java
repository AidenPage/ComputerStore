...
public class StorageDeviceRepositoryImpl extends SQLiteOpenHelper implements StorageDeviceRepository {
    ...
    private static final int DATABASE_VERSION = DBConstants.DATABASE_VERSION+12;
    ...
    public StorageDeviceRepositoryImpl(Context context) {
    ...
    public StorageDevice findById(Long id) {
        ...
        if (cursor.moveToFirst()) {
            final StorageDevice storageDevice = new StorageDevice.Builder()
    ...
    @Override
    public StorageDevice save(StorageDevice entity) {
        open();
        ...
        StorageDevice insertedEntity = new StorageDevice.Builder()
    ...
    @Override
    public StorageDevice delete(StorageDevice entity) {
        open();
        db.delete(
                TABLE_NAME,
                COLUMN_ID + " =? ",
    ...
    public Set<StorageDevice> findAll() {
        SQLiteDatabase db = this.getReadableDatabase();
        Set<StorageDevice> storageDevice = new HashSet<>();
        open();
        Cursor cursor = db.query(TABLE_NAME, null,null,null,null,null,null);
        if (cursor.moveToFirst()) {
            do {
                final StorageDevice personAddress = new StorageDevice.Builder()
                        .id(cursor.getLong(cursor.getColumnIndex(COLUMN_ID)))
                        .description(cursor.getString(cursor.getColumnIndex(COLUMN_DESCRIPTION)))
                        .price(cursor.getDouble(cursor.getColumnIndex(COLUMN_PRICE)))
                        .stock(cursor.getInt(cursor.getColumnIndex(COLUMN_STOCK)))
                        .build();
                storageDevice.add(personAddress);
