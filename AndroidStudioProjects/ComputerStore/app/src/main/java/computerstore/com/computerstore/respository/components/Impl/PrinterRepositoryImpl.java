...
public class PrinterRepositoryImpl extends SQLiteOpenHelper implements PrinterRepository {
    ...
    private static final int DATABASE_VERSION = DBConstants.DATABASE_VERSION+10;
    ...
    public PrinterRepositoryImpl(Context context) {
    ...
    public Printer findById(Long id) {
        ...
        if (cursor.moveToFirst()) {
            final Printer printer = new Printer.Builder()
    ...
    @Override
    public Printer save(Printer entity) {
        open();
        ...
        Printer insertedEntity = new Printer.Builder()
                .Printer(entity)
                .id(new Long(id))
                .build();
    ...
    @Override
    public Printer delete(Printer entity) {
        open();
        db.delete(
                TABLE_NAME,
                COLUMN_ID + " =? ",
    ...
    @Override
    public Set<Printer> findAll() {
        SQLiteDatabase db = this.getReadableDatabase();
        Set<Printer> printer = new HashSet<>();
        open();
        Cursor cursor = db.query(TABLE_NAME, null,null,null,null,null,null);
        if (cursor.moveToFirst()) {
            do {
                final Printer personAddress = new Printer.Builder()
                        .id(cursor.getLong(cursor.getColumnIndex(COLUMN_ID)))
                        .description(cursor.getString(cursor.getColumnIndex(COLUMN_DESCRIPTION)))
                        .price(cursor.getDouble(cursor.getColumnIndex(COLUMN_PRICE)))
                        .stock(cursor.getInt(cursor.getColumnIndex(COLUMN_STOCK)))
                        .build();
                printer.add(personAddress);
