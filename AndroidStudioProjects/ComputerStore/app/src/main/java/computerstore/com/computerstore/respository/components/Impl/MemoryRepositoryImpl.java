...
public class MemoryRepositoryImpl extends SQLiteOpenHelper implements MemoryRepository {
    ...
    private static final int DATABASE_VERSION = DBConstants.DATABASE_VERSION+5;
    ...
    public MemoryRepositoryImpl(Context context) {
    ...
    public Memory findById(Long id) {
        ...
        if (cursor.moveToFirst()) {
            final Memory memory = new Memory.Builder()
    ...
    @Override
    public Memory save(Memory entity) {
        open();
        ...
        Memory insertedEntity = new Memory.Builder()
                .Memory(entity)
                .id(new Long(id))
                .build();
    ...
    @Override
    public Memory delete(Memory entity) {
        open();
        db.delete(
                TABLE_NAME,
                COLUMN_ID + " =? ",
    ...
    public Set<Memory> findAll() {
        ...
        if (cursor.moveToFirst()) {
            do {
                final Memory personAddress = new Memory.Builder()
                        .id(cursor.getLong(cursor.getColumnIndex(COLUMN_ID)))
                        .description(cursor.getString(cursor.getColumnIndex(COLUMN_DESCRIPTION)))
                        .price(cursor.getDouble(cursor.getColumnIndex(COLUMN_PRICE)))
                        .stock(cursor.getInt(cursor.getColumnIndex(COLUMN_STOCK)))
                        .build();
                memory.add(personAddress);
