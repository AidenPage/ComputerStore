...
public class DisplayCardRepositoryImpl extends SQLiteOpenHelper implements DisplayCardRepository {
    public static final String TABLE_NAME = "display_card";
    ...
    private static final int DATABASE_VERSION = DBConstants.DATABASE_VERSION+3;
    ...
    public DisplayCardRepositoryImpl(Context context) {
    ...
    @Override
    public DisplayCard findById(Long id) {
        ...
        if (cursor.moveToFirst()) {
            final DisplayCard displayCard = new DisplayCard.Builder()
    ...
    @Override
    public DisplayCard save(DisplayCard entity) {
        open();
        ...
        DisplayCard insertedEntity = new DisplayCard.Builder()
                .DisplayCard(entity)
                .id(new Long(id))
                .build();
    ...
    @Override
    public DisplayCard delete(DisplayCard entity) {
        open();
        db.delete(
                TABLE_NAME,
                COLUMN_ID + " =? ",
    ...
    @Override
    public Set<DisplayCard> findAll() {
        SQLiteDatabase db = this.getReadableDatabase();
        Set<DisplayCard> displayCard = new HashSet<>();
        open();
        Cursor cursor = db.query(TABLE_NAME, null,null,null,null,null,null);
        if (cursor.moveToFirst()) {
            do {
                final DisplayCard personAddress = new DisplayCard.Builder()
                        .id(cursor.getLong(cursor.getColumnIndex(COLUMN_ID)))
                        .description(cursor.getString(cursor.getColumnIndex(COLUMN_DESCRIPTION)))
                        .price(cursor.getDouble(cursor.getColumnIndex(COLUMN_PRICE)))
                        .stock(cursor.getInt(cursor.getColumnIndex(COLUMN_STOCK)))
                        .build();
                displayCard.add(personAddress);
