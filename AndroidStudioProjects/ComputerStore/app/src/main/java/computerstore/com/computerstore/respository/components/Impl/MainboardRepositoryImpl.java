...
public class MainboardRepositoryImpl extends SQLiteOpenHelper implements MainboardRepository {
    public static final String TABLE_NAME = "main_board";
    ...
    private static final int DATABASE_VERSION = DBConstants.DATABASE_VERSION+4;
    ...
    public MainboardRepositoryImpl(Context context) {
    ...
    public Mainboard findById(Long id) {
        ...
        if (cursor.moveToFirst()) {
            final Mainboard mainboard = new Mainboard.Builder()
    ...
    @Override
    public Mainboard save(Mainboard entity) {
        open();
        ...
        Mainboard insertedEntity = new Mainboard.Builder()
                .Mainboard(entity)
                .id(new Long(id))
                .build();
    ...
    @Override
    public Mainboard delete(Mainboard entity) {
        open();
        db.delete(
                TABLE_NAME,
                COLUMN_ID + " =? ",
    ...
    @Override
    public Set<Mainboard> findAll() {
        SQLiteDatabase db = this.getReadableDatabase();
        Set<Mainboard> mainboard = new HashSet<>();
        open();
        Cursor cursor = db.query(TABLE_NAME, null,null,null,null,null,null);
        if (cursor.moveToFirst()) {
            do {
                final Mainboard personAddress = new Mainboard.Builder()
                        .id(cursor.getLong(cursor.getColumnIndex(COLUMN_ID)))
                        .description(cursor.getString(cursor.getColumnIndex(COLUMN_DESCRIPTION)))
                        .price(cursor.getDouble(cursor.getColumnIndex(COLUMN_PRICE)))
                        .stock(cursor.getInt(cursor.getColumnIndex(COLUMN_STOCK)))
                        .build();
                mainboard.add(personAddress);
